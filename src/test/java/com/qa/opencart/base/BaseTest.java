...
public class BaseTest {
	...
	protected LoginPage loginpage;   // reference variable of LoginPage class/ making it protected because only
	                                // child class in other package can access it not all the member.and also making it basetest class ref variable because all Base test class child can access it.
	...
	// we are not going to initialize driver here again because it is already in initialize in driver factory class
	// we just need to call driver initialize method here
	// so to call the method we have to create object of driver factory class
	// creating driver factory object reference at the class level so that can use it
